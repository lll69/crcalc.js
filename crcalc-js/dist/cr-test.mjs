...
class CRTest {
    ...
    testCR() {
        ...
        CRTest.check_eq(one.divide(thirteen).multiply(thirteen), one, "divide failed 2");
        ...
        CRTest.check(e.toStringD(20).substring(0, 17)
            == ("2.718281828459045"), "exp(1) failed");
        ...
        CRTest.check_eq(asin.execute(one.negate()), half_pi.negate(), "asin(-1) failed");
        ...
        CRTest.check_eq(atan.execute(tan.execute(one.negate())), one.negate(), "atan(tan(-1) failed");
        CRTest.check_eq(tan.execute(atan.execute(huge)), huge, "tan(atan(10**15)) failed");
        ...
        for (let n = -10.0; n < 10.0; n += 2.0) {
            CRTest.check_appr_eq(Math.sin(n), CR.valueOfS(String(n)).sin().doubleValue(), "sin failed at " + n);
            CRTest.check_appr_eq(Math.cos(n), CR.valueOfS(String(n)).cos().doubleValue(), "cos failed at " + n);
            CRTest.check_appr_eq(Math.exp(n), CR.valueOfS(String(n)).exp().doubleValue(), "exp failed at " + n);
            CRTest.check_appr_eq(Math.asin(0.1 * n), CR.valueOfS(String(0.1 * n)).asin().doubleValue(), "asin failed at " + 0.1 * n);
            CRTest.check_appr_eq(Math.acos(0.1 * n), CR.valueOfS(String(0.1 * n)).acos().doubleValue(), "acos failed at " + 0.1 * n);
            if (n > 0.0) {
                CRTest.check_appr_eq(Math.log(n), CR.valueOfS(String(n)).ln().doubleValue(), "ln failed at " + n);
            }
        }
        CRTest.check_appr_eq(Math.cos(12345678.0), CR.valueOfN(12345678n).cos().doubleValue(), "cos failed at " + 12345678);
        console.log("Success");
    }
}
new CRTest().testCR();
